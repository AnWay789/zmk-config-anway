...
static void raise_changed(void) {
    raise_zmk_trackpad_cal_state_changed((struct zmk_trackpad_cal_state_changed){.angle = angle});
}
...
void trackpad_cal_reset(void) { trackpad_cal_set(DEFAULT_ANGLE); }
...
static int tpr_handle_event(const struct device *dev, struct input_event *event, uint32_t param1,
    ...
    if (event->code == INPUT_REL_X) {
        ...
        int32_t y = data->y;
        data->x = event->value;
        event->value = (x * cos_val - y * sin_val) / ANGLE_SCALE;
    } else if (event->code == INPUT_REL_Y) {
        int32_t x = data->x;
        int32_t y = event->value;
        data->y = event->value;
        event->value = (x * sin_val + y * cos_val) / ANGLE_SCALE;
    }
